...
class Connection extends Component
{
    ...
    public function open()
    {
        ...
        if ($this->_socket) {
            ...
            \Log::fatal("redis_request", 'monitor', ['status'=>'failed','server'=>$this->hostname . ':' . $this->port,'error'=>$message]);
            return null;
        }
    }
    ...
    private function parseResponse($command)
    {
        if (($line = fgets($this->_socket)) === false) {
            ...
            $message = "Failed to read from socket,Redis command was: " . $command;
            \Log::fatal("redis_request", 'monitor', ['status'=>'failed','server'=>$this->hostname . ':' . $this->port,'error'=>$message]);
            return null;
        }
        $type = $line[0];
        ...
        switch ($type) {
            ...
            case '-': // Error reply
                ...
                $message = "Redis error: " . $line . ",Redis command was: " . $command;
                \Log::fatal("redis_request", 'monitor', ['status'=>'failed','server'=>$this->hostname . ':' . $this->port,'error'=>$message]);
            case ':': // Integer reply
            ...
            case '$': // Bulk replies
                ...
                while ($length > 0) {
                    if (($block = fread($this->_socket, $length)) === false) {
                        // throw new Exception("Failed to read from socket.\nRedis command was: " . $command);
                        $message = "Failed to read from socket,Redis command was: " . $command;
                        \Log::fatal("redis_request", 'monitor', ['status'=>'failed','error'=>$message]);
                        return null;
                    }
                    $data .= $block;
            ...
            default:
                ...
                $message = 'Received illegal data from redis: ' . $line . ",Redis command was: " . $command;
                \Log::fatal("redis_request", 'monitor', ['status'=>'failed','server'=>$this->hostname . ':' . $this->port,'error'=>$message]);
        }
    }
}
